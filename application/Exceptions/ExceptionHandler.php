...
class ExceptionHandler extends Exception
{
    ...
    {
        // TODO! Extension for Database module and query display ?
        if ($exception instanceof QueryException) {
            $this->renderQueryException($exception, $env);
            return;
        }
    ...
    private function renderQueryException(QueryException $exception, string $env): void
    {
        $sql = htmlspecialchars($exception->sql, ENT_QUOTES);
        $params = json_encode($exception->params, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        $paramsEnc = htmlspecialchars($params, ENT_QUOTES);

        if ($env === 'development') {
            $this->renderDetailedError(
                500,
                "<span>Database Query Error!<span><br><strong>SQL:</strong> <span>{$sql}</span><br><strong>Params:</strong> <span>{$paramsEnc}</span>",
                $exception->getFile(),
                $exception->getLine(),
                $exception->getTraceAsString()
            );
        } else {
            $this->renderProductionError(500);
        }
    }
}
