	...
	<script>
		$(document).ready(function() {
			$('#sectionDelete').click(function () {
				...
				$.ajax({
					type: "GET",
					url: '<?php echo APP_PATH; ?>panel/ajaxDeleteSection',
					data: params,
					success: function(response) {
						if (response != 0) {
							var alert = JSON.parse(response);
							window.location.replace("<?php echo APP_PATH; ?>panel/manageBlogSections?action=delete" + "&status=" + alert['status'] + '&message=' + alert['message']);
						} else {
